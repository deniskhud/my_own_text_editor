...
#define KHUD_TAB_STOP 8
#define KHUD_QUIT_TIMES 3
...
struct editorSyntax {
	...
	char* multiple_comment_start;
	char* multiple_comment_end;
	int flags;
};
...
void editorUpdateSyntax(erow* row) {
	...
	char* mcs = E.syntax->multiple_comment_start;
	char* mce = E.syntax->multiple_comment_end;
...
int editorRowCxToRx(erow* row, int cursorX){
	...
	for(int i = 0; i < cursorX; ++i){
		if(row->chars[i] == '\t'){
			rx += (KHUD_TAB_STOP - 1) - (rx % KHUD_TAB_STOP);
		}
		++rx;
	}
	return rx;
...
void editorUpdateRow(erow* row){
	...
	free(row->render);
	row->render = malloc(row->size + tabs * (KHUD_TAB_STOP - 1) + 1);

	int index = 0;
	for(int i = 0; i < row->size; ++i){
		if(row->chars[i] == '\t'){
			row->render[index++] = ' ';
			while(index % KHUD_TAB_STOP != 0) row->render[index++] = ' ';
		}
		else{
...
void EditorDrawStatusbar(struct abuf* ab){
	...
	int rlen = snprintf(rstatus, sizeof(rstatus), "%s | %d/%d",
		E.syntax ? E.syntax->filetype : "no ft", E.cursorY + 1, E.numrows);
...
void editorDrawMessageBar(struct abuf* ab){
	...
	if(msglen > E.screenrows) msglen = E.screencols;
...
void editorProcessKeypress(){
	static int quit_times = KHUD_QUIT_TIMES;
	int c = editorReadKey();
